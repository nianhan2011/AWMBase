...
#import "AWMInitializationUIMethod.h"
...
{
    UIButton *btn = InsertBackGroundImageButton(superView, rc, tag, img, imgH, target, action);
...
{
    UIButton *btn = InsertImageViewButton(superView, rc, tag, img, imgH, target, action);
...
NSMutableAttributedString *modifyDigital(NSString *context, UIColor *nubColor,UIFont *nubFont,UIColor *normalColor,UIFont *normalFont)
{
    ...
    NSArray<NSTextCheckingResult *> *ranges = [regular matchesInString:context options:0 range:NSMakeRange(0, [context length])];
    NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc] initWithString:context attributes:@{NSForegroundColorAttributeName:normalColor,NSFontAttributeName: normalFont}];
    for (int i = 0; i < ranges.count; i++) {
        [attStr setAttributes:@{NSForegroundColorAttributeName : nubColor,NSFontAttributeName: nubFont} range:ranges[i].range];
    }
    return attStr;
}
